...
(async () => {
  ...
  if (!process.argv.includes("-offline")) {
    ...
    const remotepkg = await axios.get(
  ...
  if (process.argv.includes("-offline")) {
    ...
    if (type === "lib") {
      ...
      const lib = await axios.get("https://api.mcbuild.dev/lib/" + name)
        .then((res) => res.data)
        .catch((e) => {
      ...
      const lang = await axios.get("https://api.mcbuild.dev/lang/" + name)
        .then((res) => res.data)
        .catch((e) => {
      ...
      for (let i = 0; i < names.length; i++) {
        const lang = await axios.get("https://api.mcbuild.dev/lang/" + names[i])
        .then((res) => res.data)
        .catch((e) => {
