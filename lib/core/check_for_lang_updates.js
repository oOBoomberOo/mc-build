...
(async () => {
  ...
  const os = require("os");
  const LOCAL_DIR = os.platform().indexOf("win")!==-1?path.resolve(
    process.env.APPDATA,
    "mc-build",
    "local"
  ):path.resolve(
              ...
              if (data.hasOwnProperty('message') && data.message.startsWith("API rate limit exceeded")){
                fetch("https://api.github.com/rate_limit", {
                  headers: {
                    ...
                    try{
                      throw new Error(`Wait until ${new Date(resetTime * 1e3).toISOString().slice(-13, -5)} or use the -offline to bypass the update check if you have the languages downloaded already`);
                    } catch (e) {
